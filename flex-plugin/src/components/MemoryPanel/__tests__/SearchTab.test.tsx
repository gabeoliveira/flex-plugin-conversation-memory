...
jest.mock('../../../api/captureTurn', () => ({ captureTurn: jest.fn() }));
jest.mock('../../../utils/flexToken', () => ({ getAgentTraits: () => ({ email: 'a@b.com' }) }));
...
const mockCaptureTurn = captureTurn as jest.MockedFunction<typeof captureTurn>;
...
describe('SearchTab', () => {
  ...
  it('fires an agent-productivity capture on search completion and on summarize', async () => {
    mockFetchMemory.mockResolvedValue({
      identifier: '+55',
      matchedBy: 'phone',
      profileId: 'mem_profile_1',
      profileCreatedAt: null,
      traits: {},
      observations: [{ id: 'o1', content: 'Prefers mornings', createdAt: 't', score: 0.7 }],
      summaries: [],
    });
    mockSearchKnowledge.mockResolvedValue({ query: 'refund', chunks: [{ content: 'Refunds 30 days', score: 0.6 }] });
    mockSummarize.mockResolvedValue({ answer: 'Refunds are 30 days [K1].', model: 'gpt-4o-mini', grounded: true });

    renderSearch();
    submit('refund');

    // search turn captured once both searches settle
    await waitFor(() =>
      expect(mockCaptureTurn).toHaveBeenCalledWith(
        expect.objectContaining({ kind: 'search', query: 'refund', token: 'tok' }),
      ),
    );
